...
let state = {
  opportunities:[], scheduleEvents:[], emails:[], emailSettings:{}, profile:{}, experiences:[], versions:[], interviewQuestions:[], interviewAnalysis:{}, roleFieldSets:[], dataStatus:{}, syncStatus:{}, health:{}, latestVersion:null,
  memoEditing:null, memoScope:'all', emailFilter:'pending', interviewSessionView:'', writtenPaperView:'', recommendations:[], recommendationDefaultIds:[], recommendationSignature:'',
...
async function loadAll(){
  try{
    const [ops,schedule,emailMessages,emailSettings,profile,exps,versions,questions,questionAnalysis,fieldSets,dataStatus,syncStatus,health]=await Promise.all([
      api('/api/opportunities'), api('/api/schedule-events'), api('/api/email/messages'), api('/api/email/settings'), api('/api/profile'), api('/api/experiences'), api('/api/resume-versions'), api('/api/interview-questions'), api('/api/interview-questions/analysis'), api('/api/role-field-sets'), api('/api/data/status'), api('/api/sync/status'), api('/api/health')
    ]);
    ...
    state.interviewAnalysis=questionAnalysis||{};
...
function renderRoleFieldSets(){const list=$('#roleFieldSetList');if(!list)return;list.innerHTML=state.roleFieldSets.map(x=>`<article class="role-field-set-card" data-id="${x.id}"><div class="interview-question-head"><div><span class="tag">${esc(x.role_category||'未分类岗位')}</span><b>${esc(x.title||'自我评价')}</b></div></div><p class="self-evaluation-copy">${esc(x.self_evaluation||'暂未填写自我评价。')}</p><div class="memo-actions"><button class="small-btn" data-action="edit-field-set">编辑</button><button class="small-btn danger" data-action="delete-field-set">删除</button></div></article>`).join('')||'<div class="empty-state compact">还没有自我评价，点击“新增自我评价”按岗位类别建立第一条。</div>';}
function openRoleFieldSet(item=null){const x=item||{};$('#roleFieldSetId').value=x.id||'';$('#roleFieldSetDialogTitle').textContent=item?'编辑自我评价':'新增自我评价';$('#roleFieldSetRole').value=x.role_category||'';$('#roleFieldSetTitle').value=x.title||'';$('#roleFieldSetSelfEvaluation').value=x.self_evaluation||'';$('#roleFieldSetDialog').showModal();}
...
function goView(view){const viewId=`view-${view.replace(/_/g,'-')}`;$$('.nav').forEach(x=>x.classList.toggle('active',x.dataset.view===view));$$('.view').forEach(x=>x.classList.toggle('active-view',x.id===viewId));$('#viewTitle').textContent=titles[view]||'';$('#viewSubtitle').textContent=subtitles[view]||'集中查看岗位进展、待办事项和简历准备情况。';if(view==='vault'){vaultView='experiences';$$('[data-vault-view]').forEach(x=>x.classList.toggle('active',x.dataset.vaultView===vaultView));sendVaultView(vaultView);}}
function sendVaultView(view){const frame=$('#careerVaultFrame'),panel=$('#selfEvaluationsPanel');if(!frame)return;vaultView=view;const evaluations=view==='evaluations';frame.classList.toggle('hidden',evaluations);panel?.classList.toggle('hidden',!evaluations);if(!evaluations)frame.contentWindow?.postMessage({type:'careeros-vault-view',view},'*');}
function reloadCareerVaultFrame(){const frame=$('#careerVaultFrame');if(!frame)return;frame.src=`http://127.0.0.1:8776/?embedded=1&v=${Date.now()}`;}
...
$('#addInterviewQuestionBtn').addEventListener('click',()=>openInterviewQuestion(null,'interview'));$('#addWrittenQuestionBtn').addEventListener('click',()=>openInterviewQuestion(null,'written_test'));$('#interviewQuestionClose').addEventListener('click',()=>$('#interviewQuestionDialog').close());$('#interviewQuestionCancel').addEventListener('click',()=>$('#interviewQuestionDialog').close());$('#interviewQuestionType').addEventListener('change',e=>syncQuestionFormType(e.target.value));
['interviewRoleFilter','interviewTopicFilter','interviewSourceFilter'].forEach(id=>$('#'+id).addEventListener('change',renderInterviewQuestions));$('#interviewSearch').addEventListener('input',renderInterviewQuestions);$('#clearInterviewFilters').addEventListener('click',()=>{['interviewRoleFilter','interviewTopicFilter','interviewSourceFilter'].forEach(id=>$('#'+id).value='');$('#interviewSearch').value='';renderInterviewQuestions();});$('#backToInterviewSessions').addEventListener('click',()=>{state.interviewSessionView='';renderInterviewQuestions();});$('#toggleAllInterviewAnswers').addEventListener('click',()=>{const answers=$$('#interviewSessionQuestions .interview-prep');const shouldShow=answers.some(answer=>answer.classList.contains('hidden'));answers.forEach(answer=>answer.classList.toggle('hidden',!shouldShow));$$('#interviewSessionQuestions [data-action="toggle-interview-prep"]').forEach(button=>button.textContent=shouldShow?'收起准备提示':'显示准备提示');$('#toggleAllInterviewAnswers').textContent=shouldShow?'收起全部准备提示':'显示全部准备提示';});
['writtenRoleFilter','writtenTopicFilter','writtenSourceFilter'].forEach(id=>$('#'+id).addEventListener('change',renderWrittenQuestions));$('#writtenSearch').addEventListener('input',renderWrittenQuestions);$('#clearWrittenFilters').addEventListener('click',()=>{['writtenRoleFilter','writtenTopicFilter','writtenSourceFilter'].forEach(id=>$('#'+id).value='');$('#writtenSearch').value='';renderWrittenQuestions();});$('#backToWrittenPapers').addEventListener('click',()=>{state.writtenPaperView='';renderWrittenQuestions();});$('#toggleAllWrittenAnswers').addEventListener('click',()=>{const answers=$$('#writtenPaperQuestions .question-answer');const shouldShow=answers.some(answer=>answer.classList.contains('hidden'));answers.forEach(answer=>answer.classList.toggle('hidden',!shouldShow));$$('#writtenPaperQuestions [data-action="toggle-written-answer"]').forEach(button=>button.textContent=shouldShow?'收起答案与解析':'查看答案与解析');$('#toggleAllWrittenAnswers').textContent=shouldShow?'收起全部答案':'显示全部答案';});
$('#interviewQuestionForm').addEventListener('submit',async e=>{e.preventDefault();const id=$('#interviewQuestionId').value;const options=$('#interviewQuestionOptions').value.split('\n').map(line=>{const match=line.trim().match(/^([A-D])\.\s*(.+)$/);return match?{key:match[1],text:match[2].trim()}:null;}).filter(Boolean);const payload={question_type:$('#interviewQuestionType').value,source_type:$('#interviewQuestionSource').value,source_key:id?undefined:'manual:'+Date.now(),paper_name:$('#interviewQuestionPaper').value.trim(),question_no:Number($('#interviewQuestionNo').value)||0,topic:$('#interviewQuestionTopic').value.trim(),role_category:$('#interviewQuestionRole').value.trim(),company:$('#interviewQuestionCompany').value.trim(),event_date:$('#interviewQuestionDate').value,opportunity_id:Number($('#interviewQuestionOpportunity').value)||null,question:$('#interviewQuestionText').value.trim(),options,correct_answer:$('#interviewQuestionCorrect').value.trim(),answer:$('#interviewQuestionAnswer').value.trim(),analysis:$('#interviewQuestionAnalysis').value.trim(),feeling:$('#interviewQuestionFeeling').value.trim(),tags:$('#interviewQuestionTags').value.split(/[,，]/).map(x=>x.trim()).filter(Boolean)};if(payload.question_type==='interview'){payload.options=[];payload.correct_answer='';}if(!payload.source_key)delete payload.source_key;try{await api(id?`/api/interview-questions/${id}`:'/api/interview-questions',{method:id?'PATCH':'POST',body:JSON.stringify(payload)});$('#interviewQuestionDialog').close();notice(id?'题目已更新。':'题目已保存。');await loadAll();}catch(err){notice(err.message,true);}});
async function handleQuestionCardAction(e,container){const action=e.target.dataset.action;if(!action)return;const card=e.target.closest('[data-id]');if(!card)return;const id=Number(card.dataset.id),item=state.interviewQuestions.find(question=>question.id===id);if(action==='toggle-interview-prep'||action==='toggle-written-answer'){const body=card.querySelector(action==='toggle-interview-prep'?'.interview-prep':'.question-answer');if(!body)return;const hidden=body.classList.toggle('hidden');e.target.textContent=hidden?(action==='toggle-interview-prep'?'显示准备提示':'查看答案与解析'):(action==='toggle-interview-prep'?'收起准备提示':'收起答案与解析');return;}if(action==='edit-question'&&item){openInterviewQuestion(item);return;}if(action==='delete-question'&&confirm('删除这道题目？')){try{await api(`/api/interview-questions/${id}`,{method:'DELETE'});await loadAll();}catch(err){notice(err.message,true);}}}
$('#interviewSessionList').addEventListener('click',e=>{if(e.target.dataset.action!=='open-interview-session')return;state.interviewSessionView=e.target.closest('.interview-session-card')?.dataset.sessionName||'';renderInterviewQuestions();});$('#interviewSessionQuestions').addEventListener('click',e=>handleQuestionCardAction(e,'interview'));$('#writtenQuestionList').addEventListener('click',e=>{if(e.target.dataset.action!=='open-written-paper')return;state.writtenPaperView=e.target.closest('.written-paper-card')?.dataset.paperName||'';renderWrittenQuestions();});$('#writtenPaperQuestions').addEventListener('click',e=>handleQuestionCardAction(e,'written'));
...
$('#roleFieldSetForm').addEventListener('submit',async e=>{e.preventDefault();const id=$('#roleFieldSetId').value;const payload={role_category:$('#roleFieldSetRole').value.trim(),title:$('#roleFieldSetTitle').value.trim(),self_evaluation:$('#roleFieldSetSelfEvaluation').value.trim(),strengths:'',skills:[],common_answers:{},notes:''};if(!payload.role_category)return notice('请填写岗位类别。',true);if(!payload.self_evaluation)return notice('请填写自我评价。',true);try{await api(id?`/api/role-field-sets/${id}`:'/api/role-field-sets',{method:id?'PATCH':'POST',body:JSON.stringify(payload)});$('#roleFieldSetDialog').close();notice(id?'自我评价已更新。':'自我评价已保存。');await loadAll();}catch(err){notice(err.message,true);}});
$('#roleFieldSetList').addEventListener('click',async e=>{const action=e.target.dataset.action;if(!action)return;const id=Number(e.target.closest('.role-field-set-card').dataset.id),item=state.roleFieldSets.find(x=>x.id===id);if(action==='edit-field-set')openRoleFieldSet(item);if(action==='delete-field-set'&&confirm('删除这条自我评价？')){await api(`/api/role-field-sets/${id}`,{method:'DELETE'});await loadAll();}});

// ---------- separated interview / written-test banks ----------
function questionGroupName(item,type){
  if(item.paper_name)return item.paper_name;
  if(type==='interview')return `${item.company||'未命名面试'}${item.event_date?` · ${item.event_date}`:''}`;
  return '未命名试卷';
}
function questionGroups(items,type){
  const groups=new Map();
  items.forEach(item=>{const name=questionGroupName(item,type);if(!groups.has(name))groups.set(name,[]);groups.get(name).push(item);});
  return [...groups.entries()].map(([name,rows])=>[name,rows.sort((a,b)=>(a.question_no||999999)-(b.question_no||999999)||String(a.created_at||'').localeCompare(String(b.created_at||''))) ]);
}
function fillQuestionFilter(selector,label,values){
...
function questionFilter(type,prefix){
  const role=$(`#${prefix}RoleFilter`)?.value||'';
  const topic=$(`#${prefix}TopicFilter`)?.value||'';
  const source=$(`#${prefix}SourceFilter`)?.value||'';
  const search=($(`#${prefix}Search`)?.value||'').trim().toLowerCase();
  const rows=state.interviewQuestions.filter(item=>{
    if(item.question_type!==type||role&&item.role_category!==role||topic&&item.topic!==topic||source&&item.source_type!==source)return false;
    const hay=[item.question,item.answer,item.analysis,item.feeling,item.paper_name,item.topic,item.company,item.role_category,...(item.tags||[])].join(' ').toLowerCase();
    return !search||hay.includes(search);
  });
  return {rows,roles:[...new Set(state.interviewQuestions.filter(item=>item.question_type===type).map(item=>item.role_category).filter(Boolean))],topics:[...new Set(state.interviewQuestions.filter(item=>item.question_type===type).map(item=>item.topic).filter(Boolean))]};
}
function sourceLabel(source){return source==='personal'?'亲身经历':'网络题库';}
function formatQuestionDate(date){return date?String(date).replace(/-/g,'/'):'';}
function displayQuestionText(value=''){return esc(value).replace(/\*\*([^*\n]+)\*\*/g,'<strong>$1</strong>').replace(/`([^`\n]+)`/g,'<code>$1</code>');}
function interviewPrepMarkup(item){
  const blocks=[];
  if(item.answer)blocks.push(`<section class="interview-prep-block response"><b>回答建议</b><p>${displayQuestionText(item.answer)}</p></section>`);
  if(item.analysis)blocks.push(`<section class="interview-prep-block focus"><b>考察重点</b><p>${displayQuestionText(item.analysis)}</p></section>`);
  if(item.feeling)blocks.push(`<section class="interview-prep-block reflection"><b>我的复盘</b><p>${displayQuestionText(item.feeling)}</p></section>`);
  return blocks.join('');
}
function interviewQuestionCard(item,index){
  const prep=interviewPrepMarkup(item);
  return `<article class="interview-question-card interview-question-card-new" data-id="${item.id}">
    <div class="interview-card-head"><span class="interview-question-index">Q${String(index+1).padStart(2,'0')}</span><div class="interview-card-labels"><span class="topic-tag">${esc(item.topic||'未分类')}</span>${item.event_date?`<span class="helper">${esc(formatQuestionDate(item.event_date))}</span>`:''}</div><div class="interview-card-actions"><button class="link-button" type="button" data-action="edit-question">编辑</button><button class="link-button danger-link" type="button" data-action="delete-question">删除</button></div></div>
    <h3>${esc(item.question||'未填写问题')}</h3>
    ${prep?`<button class="interview-reveal" type="button" data-action="toggle-interview-prep">显示准备提示</button><div class="interview-prep hidden">${prep}</div>`:`<div class="interview-missing">还没有补充回答建议，点击“编辑”完善这道题。</div>`}
  </article>`;
}
function renderInterviewQuestions(){
  const list=$('#interviewSessionList');if(!list)return;
  const result=questionFilter('interview','interview');
  fillQuestionFilter('#interviewRoleFilter','全部岗位类别',result.roles);
  fillQuestionFilter('#interviewTopicFilter','全部主题',result.topics);
  const groups=questionGroups(result.rows,'interview');
  const allGroups=questionGroups(state.interviewQuestions.filter(item=>item.question_type==='interview'),'interview');
  $('#interviewQuestionCount').textContent=`${groups.length} 场 · ${result.rows.length} 题`;
  $('#interviewStatSessions').textContent=allGroups.length;
  $('#interviewStatTotal').textContent=state.interviewQuestions.filter(item=>item.question_type==='interview').length;
  $('#interviewStatAnswered').textContent=state.interviewQuestions.filter(item=>item.question_type==='interview'&&(item.answer||item.analysis||item.feeling)).length;
  $('#interviewAnalysisHint').textContent=allGroups.length?`${allGroups.length} 场面试 · 按场次查看`:'';
  list.innerHTML=groups.map(([name,items])=>{
    const first=items[0]||{};const topics=[...new Set(items.map(item=>item.topic).filter(Boolean))];const prepared=items.filter(item=>item.answer||item.analysis||item.feeling).length;
    return `<article class="interview-session-card" data-session-name="${esc(name)}"><div class="session-card-top"><span class="session-kind">面试复盘</span><span class="helper">${items.length} 道问题</span></div><h3>${esc(name)}</h3><p class="session-meta">${esc(first.company||'未填写公司')}${first.role_category?` · ${esc(first.role_category)}`:''}${first.event_date?` · ${esc(formatQuestionDate(first.event_date))}`:''}</p><div class="session-topic-list">${topics.length?topics.slice(0,5).map(topic=>`<span>${esc(topic)}</span>`).join(''):'<span>待补充主题</span>'}</div><div class="session-card-foot"><span class="helper">${prepared}/${items.length} 题已整理 · ${sourceLabel(first.source_type)}</span><button class="primary-btn small" type="button" data-action="open-interview-session">查看面试记录 →</button></div></article>`;
  }).join('');
  $('#interviewQuestionEmpty').classList.toggle('hidden',groups.length>0);
  if(state.interviewSessionView)renderInterviewSession(state.interviewSessionView);else{$('#interviewLibraryView')?.classList.remove('hidden');$('#interviewSessionDetail')?.classList.add('hidden');}
  renderWrittenQuestions();
}
function renderInterviewSession(name){
  const detail=$('#interviewSessionDetail'),library=$('#interviewLibraryView');if(!detail||!library)return;
  const items=state.interviewQuestions.filter(item=>item.question_type==='interview'&&questionGroupName(item,'interview')===name).sort((a,b)=>(a.question_no||999999)-(b.question_no||999999));
  if(!items.length){state.interviewSessionView='';library.classList.remove('hidden');detail.classList.add('hidden');return;}
  library.classList.add('hidden');detail.classList.remove('hidden');
  const first=items[0]||{};$('#interviewSessionTitle').textContent=name;$('#interviewSessionMeta').textContent=`${first.company||'未填写公司'}${first.role_category?` · ${first.role_category}`:''}${first.event_date?` · ${formatQuestionDate(first.event_date)}`:''} · ${items.length} 道问题`;
  $('#interviewSessionQuestions').innerHTML=items.map((item,index)=>interviewQuestionCard(item,index)).join('');
  $('#toggleAllInterviewAnswers').textContent='显示全部准备提示';
}
function writtenQuestionMarkup(item,index){
  const options=(item.options||[]).map(option=>`<li><span>${esc(option.key||'')}</span><div>${displayQuestionText(option.text||'')}</div></li>`).join('');
  const hasAnswer=item.correct_answer||item.answer||item.analysis||item.feeling;
  const answer=hasAnswer?`<button class="link-button reveal-answer" type="button" data-action="toggle-written-answer">查看答案与解析</button><div class="question-answer hidden">${item.correct_answer?`<div class="question-block correct-answer"><b>正确选项</b><p>${displayQuestionText(item.correct_answer)}</p></div>`:''}${item.answer?`<div class="question-block"><b>答案 / 参考答案</b><p>${displayQuestionText(item.answer)}</p></div>`:''}${item.analysis?`<div class="question-block analysis-block"><b>解题思路</b><p>${displayQuestionText(item.analysis)}</p></div>`:''}${item.feeling?`<div class="question-block feeling"><b>复盘感受</b><p>${displayQuestionText(item.feeling)}</p></div>`:''}</div>`:'';
  return `<article class="paper-question written-paper-question" data-id="${item.id}"><div class="paper-question-label"><span>第 ${item.question_no||index+1} 题</span><b>${options?'选择题':'编程 / 主观题'}</b>${item.topic?`<em>${esc(item.topic)}</em>`:''}<span class="paper-question-actions"><button class="link-button" type="button" data-action="edit-question">编辑</button><button class="link-button danger-link" type="button" data-action="delete-question">删除</button></span></div><h3>${displayQuestionText(item.question||'未填写题目')}</h3>${options?`<ol class="question-options">${options}</ol>`:''}${answer}</article>`;
}
function renderWrittenQuestions(){
  const list=$('#writtenQuestionList');if(!list)return;
  const result=questionFilter('written_test','written');
  fillQuestionFilter('#writtenRoleFilter','全部岗位类别',result.roles);
  fillQuestionFilter('#writtenTopicFilter','全部主题',result.topics);
  const groups=questionGroups(result.rows,'written_test');const all=state.interviewQuestions.filter(item=>item.question_type==='written_test');const allGroups=questionGroups(all,'written_test');
  $('#writtenQuestionCount').textContent=`${groups.length} 套 · ${result.rows.length} 题`;
  $('#writtenStatPapers').textContent=allGroups.length;$('#writtenStatTotal').textContent=all.length;$('#writtenStatAnswered').textContent=all.filter(item=>item.correct_answer||item.answer||item.analysis).length;$('#writtenAnalysisHint').textContent=allGroups.length?`${allGroups.length} 套试卷 · 可按主题复习`:'';
  list.innerHTML=groups.map(([name,items])=>{const first=items[0]||{};const topics=[...new Set(items.map(item=>item.topic).filter(Boolean))];const answered=items.filter(item=>item.correct_answer||item.answer||item.analysis).length;return `<article class="interview-paper-card written-paper-card" data-paper-name="${esc(name)}"><div class="paper-card-mark"><span>笔试</span><strong>${String(items.length).padStart(2,'0')}</strong><small>题</small></div><div class="paper-card-main"><div class="interview-question-head"><div><span class="tag paper-tag">${answered}/${items.length} 题有解析</span>${first.company?`<span class="helper">${esc(first.company)}</span>`:''}</div><span class="helper">${esc(first.role_category||'综合岗位')}</span></div><h3>${esc(name)}</h3><p class="paper-card-topics">${topics.length?topics.map(topic=>`<span>${esc(topic)}</span>`).join(''):'暂无主题分类'}</p><div class="paper-card-foot"><span class="helper">${items.filter(item=>item.options?.length).length} 题含选项 · ${sourceLabel(first.source_type)}</span><button class="primary-btn small" type="button" data-action="open-written-paper">进入试卷</button></div></div></article>`;}).join('');
  $('#writtenQuestionEmpty').classList.toggle('hidden',groups.length>0);
  if(state.writtenPaperView)renderWrittenPaper(state.writtenPaperView);else{$('#writtenLibraryView')?.classList.remove('hidden');$('#writtenPaperDetail')?.classList.add('hidden');}
}
function renderWrittenPaper(name){
  const detail=$('#writtenPaperDetail'),library=$('#writtenLibraryView');if(!detail||!library)return;
  const items=state.interviewQuestions.filter(item=>item.question_type==='written_test'&&questionGroupName(item,'written_test')===name).sort((a,b)=>(a.question_no||999999)-(b.question_no||999999));
  if(!items.length){state.writtenPaperView='';library.classList.remove('hidden');detail.classList.add('hidden');return;}
  library.classList.add('hidden');detail.classList.remove('hidden');const first=items[0]||{};$('#writtenPaperTitle').textContent=name;$('#writtenPaperMeta').textContent=`${first.company||'未填写公司'}${first.role_category?` · ${first.role_category}`:''} · 共 ${items.length} 题`;$('#writtenPaperQuestions').innerHTML=items.map((item,index)=>writtenQuestionMarkup(item,index)).join('');$('#toggleAllWrittenAnswers').textContent='显示全部答案';
}
function syncQuestionFormType(type){
  const interview=type==='interview';$('#writtenQuestionFields').classList.toggle('hidden',interview);$('#interviewQuestionFormHint').textContent=interview?'记录现场问题、回答建议和面试后的复盘。':'记录题干、选项和解析，方便之后按试卷刷题。';$('#interviewQuestionDialogTitle').textContent=interview?'记录面试题':'记录笔试题';$('#interviewAnswerFieldLabel span').textContent=interview?'回答建议 / 参考表达':'答案 / 参考答案';$('#interviewAnalysisFieldLabel span').textContent=interview?'考察点 / 面试官关注':'解析';$('#interviewQuestionAnswer').placeholder=interview?'整理一版你下次可以直接表达的回答。':'';$('#interviewQuestionAnalysis').placeholder=interview?'记录面试官在意的能力、关键追问和回答要点。':'这道题考察什么，为什么选这个答案。';
}
function openInterviewQuestion(item=null,defaultType='interview'){
  const x=item||{};const type=x.question_type||defaultType;$('#interviewQuestionId').value=x.id||'';$('#interviewQuestionType').value=type;$('#interviewQuestionSource').value=x.source_type||(type==='interview'?'personal':'network');$('#interviewQuestionPaper').value=x.paper_name||'';$('#interviewQuestionNo').value=x.question_no||'';$('#interviewQuestionTopic').value=x.topic||'';$('#interviewQuestionRole').value=x.role_category||'';$('#interviewQuestionCompany').value=x.company||'';$('#interviewQuestionDate').value=x.event_date||localDate(new Date());$('#interviewQuestionText').value=x.question||'';$('#interviewQuestionOptions').value=(x.options||[]).map(option=>`${option.key||''}. ${option.text||''}`).join('\n');$('#interviewQuestionCorrect').value=x.correct_answer||'';$('#interviewQuestionAnswer').value=x.answer||'';$('#interviewQuestionAnalysis').value=x.analysis||'';$('#interviewQuestionFeeling').value=x.feeling||'';$('#interviewQuestionTags').value=(x.tags||[]).join(', ');const select=$('#interviewQuestionOpportunity');select.innerHTML='<option value="">不关联岗位</option>'+state.opportunities.map(o=>`<option value="${o.id}" ${String(o.id)===String(x.opportunity_id||'')?'selected':''}>${esc(o.company||'待补充公司')} · ${esc(o.role||o.title||'岗位')}</option>`).join('');syncQuestionFormType(type);$('#interviewQuestionDialog').showModal();
}
