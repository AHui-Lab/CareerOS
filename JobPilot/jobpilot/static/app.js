...
function renderMemo(){
  ...
  $('#memoList').innerHTML=rows.map(x=>`<article class="memo-card" data-id="${x.id}">
    ...
    <div class="meta">${x.location?`<span>${esc(x.location)}</span>`:''}${x.deadline?`<span>截止 ${esc(x.deadline)}</span>`:'<span>无明确截止时间</span>'}<span>${esc(STATUS[x.status]||'待判断')}</span><span>${esc(JOB_CATEGORY[x.job_category]||'未分类')}</span>${x.referral_code?`<span class="referral-chip">内推码：${esc(x.referral_code)}</span>`:''}</div>
...
function renderResumePreview(version){
  ...
  $('#resumePreview').innerHTML=`${sourceNote}<div class="resume-preview-heading"><div><h2>${esc(p.name||'个人简历')}</h2>${contact?`<div class="contact">${esc(contact)}</div>`:''}</div>${photo}</div>${r.headline?`<div class="headline">${esc(r.headline)}</div>`:''}${r.summary?`<p class="summary">${esc(r.summary)}</p>`:''}${sections}${(r.skills||[]).length?`<section class="resume-section"><h3>技能</h3><p>${esc(r.skills.join('、'))}</p></section>`:''}`;
