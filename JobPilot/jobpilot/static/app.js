...
let state = {
  ...
  memoEditing:null, memoScope:'all', emailFilter:'pending', interviewPaperView:'', recommendations:[], recommendationDefaultIds:[], recommendationSignature:'',
...
function renderInterviewQuestions(){
  ...
  const role=$('#interviewRoleFilter')?.value||'',type=$('#interviewTypeFilter')?.value||'',source=$('#interviewSourceFilter')?.value||'',topic=$('#interviewTopicFilter')?.value||'',search=($('#interviewSearch')?.value||'').trim().toLowerCase();
  const distinct=(items,key)=>[...new Set(items.map(x=>x[key]).filter(Boolean))];
  const roles=distinct([...state.interviewQuestions,...state.opportunities.map(x=>({role_category:x.role}))],'role_category');
  const topics=distinct(state.interviewQuestions,'topic');
  const fill=(selector,label,values)=>{const select=$(selector);if(!select)return;const current=select.value;select.innerHTML=`<option value="">${label}</option>`+values.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('');if(values.includes(current))select.value=current;};
  fill('#interviewRoleFilter','全部岗位类别',roles);fill('#interviewTopicFilter','全部主题',topics);
  const matches=x=>{const hay=[x.question,x.answer,x.analysis,x.paper_name,x.topic,...(x.tags||[])].join(' ').toLowerCase();return(!role||x.role_category===role)&&(!type||x.question_type===type)&&(!source||x.source_type===source)&&(!topic||x.topic===topic)&&(!search||hay.includes(search));};
  const groups=new Map();state.interviewQuestions.filter(matches).forEach(x=>{const name=x.paper_name||'未命名试卷';if(!groups.has(name))groups.set(name,[]);groups.get(name).push(x);});
  if($('#interviewQuestionCount'))$('#interviewQuestionCount').textContent=`${groups.size} 套试卷 · ${state.interviewQuestions.filter(matches).length} 题`;
  list.innerHTML=[...groups.entries()].map(([name,items])=>{const sorted=[...items].sort((a,b)=>(a.question_no||999999)-(b.question_no||999999));const written=sorted.filter(x=>x.question_type==='written_test').length;const interview=sorted.length-written;const topicsForPaper=[...new Set(sorted.map(x=>x.topic).filter(Boolean))];const first=sorted[0]||{};return `<article class="interview-paper-card" data-paper-name="${esc(name)}"><div class="paper-card-mark"><span>${written?'笔试':'面试'}</span><strong>${String(sorted.length).padStart(2,'0')}</strong><small>题</small></div><div class="paper-card-main"><div class="interview-question-head"><div><span class="tag">${written?`${written} 道笔试题`:''}${interview?`${written?' · ':''}${interview} 道面试题`:''}</span>${first.company?`<span class="helper">${esc(first.company)}</span>`:''}</div><span class="helper">${esc(first.role_category||'综合岗位')}</span></div><h3>${esc(name)}</h3><p class="paper-card-topics">${topicsForPaper.length?topicsForPaper.map(t=>`<span>${esc(t)}</span>`).join(''):'暂无主题分类'}</p><div class="paper-card-foot"><span class="helper">${sorted.filter(x=>x.options?.length).length} 题含选项 · ${sorted.filter(x=>x.analysis||x.correct_answer).length} 题有解析</span><button class="primary-btn small" type="button" data-action="open-paper">进入试卷</button></div></div></article>`;}).join('');
  $('#interviewQuestionEmpty').classList.toggle('hidden',groups.size>0);
  if(state.interviewPaperView)renderInterviewPaper(state.interviewPaperView);else{$('#interviewLibraryView')?.classList.remove('hidden');$('#interviewPaperDetail')?.classList.add('hidden');}
  renderInterviewAnalysis();
}
function renderInterviewPaper(name){const detail=$('#interviewPaperDetail'),library=$('#interviewLibraryView');if(!detail||!library)return;const items=state.interviewQuestions.filter(x=>(x.paper_name||'未命名试卷')===name).sort((a,b)=>(a.question_no||999999)-(b.question_no||999999));if(!items.length){state.interviewPaperView='';library.classList.remove('hidden');detail.classList.add('hidden');return;}library.classList.add('hidden');detail.classList.remove('hidden');const first=items[0]||{};$('#interviewPaperTitle').textContent=name;$('#interviewPaperMeta').textContent=`${first.company||''}${first.role_category?` · ${first.role_category}`:''} · 共 ${items.length} 题 · ${items.filter(x=>x.question_type==='written_test').length} 道笔试题`;$('#interviewPaperQuestions').innerHTML=items.map((x,index)=>{const options=(x.options||[]).map(o=>`<li><span>${esc(o.key||'')}</span><div>${esc(o.text||'')}</div></li>`).join('');const hasAnswer=x.correct_answer||x.answer||x.analysis;return `<article class="paper-question" data-id="${x.id}"><div class="paper-question-label"><span>第 ${x.question_no||index+1} 题</span><b>${x.question_type==='written_test'?'单选 / 多选'.replace('单选 / 多选',x.correct_answer?.includes('、')?'多选题':'单选题'):'面试题'}</b>${x.topic?`<em>${esc(x.topic)}</em>`:''}</div><h3>${esc(x.question)}</h3>${options?`<ol class="question-options">${options}</ol>`:''}${hasAnswer?`<button class="link-button reveal-answer" type="button" data-action="toggle-answer">查看答案与解析</button><div class="question-answer hidden">${x.correct_answer?`<div class="question-block correct-answer"><b>正确选项</b><p>${esc(x.correct_answer)}</p></div>`:''}${x.answer?`<div class="question-block"><b>答案 / 参考答案</b><p>${esc(x.answer)}</p></div>`:''}${x.analysis?`<div class="question-block analysis-block"><b>解题思路</b><p>${esc(x.analysis)}</p></div>`:''}</div>`:''}${x.feeling?`<div class="question-block feeling"><b>复盘感受</b><p>${esc(x.feeling)}</p></div>`:''}</article>`;}).join('');}
function renderInterviewAnalysis(){const a=state.interviewAnalysis||{};$('#interviewStatTotal').textContent=a.total||state.interviewQuestions.length;$('#interviewStatWritten').textContent=a.written_test||state.interviewQuestions.filter(x=>x.question_type==='written_test').length;$('#interviewStatInterview').textContent=a.interview||state.interviewQuestions.filter(x=>x.question_type==='interview').length;$('#interviewStatAnswered').textContent=a.answered||0;$('#interviewAnalysisHint').textContent=a.total?`${(a.papers||[]).length} 套试卷 · 可按主题复习`:'';const topics=a.topics||[];const max=Math.max(...topics.map(x=>x.count),1);$('#interviewTopicChart').innerHTML=topics.length?topics.map(x=>`<div class="topic-row"><span>${esc(x.name)}</span><div class="topic-bar"><i style="width:${Math.round(x.count/max*100)}%"></i></div><b>${x.count}</b></div>`).join(''):'<div class="empty-state compact">导入题目后会显示主题分布。</div>';}
function openInterviewQuestion(item=null){const x=item||{};$('#interviewQuestionId').value=x.id||'';$('#interviewQuestionDialogTitle').textContent=item?'编辑题目':'记录笔面试题目';$('#interviewQuestionType').value=x.question_type||'interview';$('#interviewQuestionSource').value=x.source_type||'personal';$('#interviewQuestionPaper').value=x.paper_name||'';$('#interviewQuestionNo').value=x.question_no||'';$('#interviewQuestionTopic').value=x.topic||'';$('#interviewQuestionRole').value=x.role_category||'';$('#interviewQuestionCompany').value=x.company||'';$('#interviewQuestionDate').value=x.event_date||localDate(new Date());$('#interviewQuestionText').value=x.question||'';$('#interviewQuestionOptions').value=(x.options||[]).map(o=>`${o.key||''}. ${o.text||''}`).join('\n');$('#interviewQuestionCorrect').value=x.correct_answer||'';$('#interviewQuestionAnswer').value=x.answer||'';$('#interviewQuestionAnalysis').value=x.analysis||'';$('#interviewQuestionFeeling').value=x.feeling||'';$('#interviewQuestionTags').value=(x.tags||[]).join(', ');const select=$('#interviewQuestionOpportunity');select.innerHTML='<option value="">不关联岗位</option>'+state.opportunities.map(o=>`<option value="${o.id}" ${String(o.id)===String(x.opportunity_id||'')?'selected':''}>${esc(o.company||'待补充公司')} · ${esc(o.role||o.title||'岗位')}</option>`).join('');$('#interviewQuestionDialog').showModal();}
...
$('#addInterviewQuestionBtn').addEventListener('click',()=>openInterviewQuestion());$('#interviewQuestionClose').addEventListener('click',()=>$('#interviewQuestionDialog').close());$('#interviewQuestionCancel').addEventListener('click',()=>$('#interviewQuestionDialog').close());['interviewRoleFilter','interviewTopicFilter','interviewTypeFilter','interviewSourceFilter'].forEach(id=>$('#'+id).addEventListener('change',renderInterviewQuestions));$('#interviewSearch').addEventListener('input',renderInterviewQuestions);$('#clearInterviewFilters').addEventListener('click',()=>{['interviewRoleFilter','interviewTopicFilter','interviewTypeFilter','interviewSourceFilter'].forEach(id=>$('#'+id).value='');$('#interviewSearch').value='';renderInterviewQuestions();});$('#backToInterviewPapers').addEventListener('click',()=>{state.interviewPaperView='';renderInterviewQuestions();});$('#toggleAllInterviewAnswers').addEventListener('click',()=>{const answers=$$('#interviewPaperQuestions .question-answer');const shouldShow=answers.some(x=>x.classList.contains('hidden'));answers.forEach(x=>x.classList.toggle('hidden',!shouldShow));$$('#interviewPaperQuestions [data-action="toggle-answer"]').forEach(x=>x.textContent=shouldShow?'收起答案与解析':'查看答案与解析');$('#toggleAllInterviewAnswers').textContent=shouldShow?'收起全部答案':'显示全部答案';});
$('#interviewQuestionForm').addEventListener('submit',async e=>{e.preventDefault();const id=$('#interviewQuestionId').value;const options=$('#interviewQuestionOptions').value.split('\n').map(line=>{const match=line.trim().match(/^([A-D])\.\s*(.+)$/);return match?{key:match[1],text:match[2].trim()}:null;}).filter(Boolean);const payload={question_type:$('#interviewQuestionType').value,source_type:$('#interviewQuestionSource').value,source_key:id?undefined:'manual:'+Date.now(),paper_name:$('#interviewQuestionPaper').value.trim(),question_no:Number($('#interviewQuestionNo').value)||0,topic:$('#interviewQuestionTopic').value.trim(),role_category:$('#interviewQuestionRole').value.trim(),company:$('#interviewQuestionCompany').value.trim(),event_date:$('#interviewQuestionDate').value,opportunity_id:Number($('#interviewQuestionOpportunity').value)||null,question:$('#interviewQuestionText').value.trim(),options,correct_answer:$('#interviewQuestionCorrect').value.trim(),answer:$('#interviewQuestionAnswer').value.trim(),analysis:$('#interviewQuestionAnalysis').value.trim(),feeling:$('#interviewQuestionFeeling').value.trim(),tags:$('#interviewQuestionTags').value.split(/[,，]/).map(x=>x.trim()).filter(Boolean)};if(!payload.source_key)delete payload.source_key;try{await api(id?`/api/interview-questions/${id}`:'/api/interview-questions',{method:id?'PATCH':'POST',body:JSON.stringify(payload)});$('#interviewQuestionDialog').close();notice(id?'题目已更新。':'题目已保存。');await loadAll();}catch(err){notice(err.message,true);}});
$('#interviewQuestionList').addEventListener('click',async e=>{const action=e.target.dataset.action;if(!action)return;if(action==='open-paper'){state.interviewPaperView=e.target.closest('.interview-paper-card')?.dataset.paperName||'';renderInterviewQuestions();return;}const card=e.target.closest('.paper-question');if(!card)return;const id=Number(card.dataset.id),item=state.interviewQuestions.find(x=>x.id===id);if(action==='toggle-answer'){const answer=card.querySelector('.question-answer');const button=e.target;const hidden=answer.classList.toggle('hidden');button.textContent=hidden?'查看答案与解析':'收起答案与解析';}if(action==='edit-question')openInterviewQuestion(item);if(action==='delete-question'&&confirm('删除这道题目？')){await api(`/api/interview-questions/${id}`,{method:'DELETE'});await loadAll();}});
$('#interviewPaperQuestions').addEventListener('click',e=>{if(e.target.dataset.action!=='toggle-answer')return;const card=e.target.closest('.paper-question'),answer=card?.querySelector('.question-answer');if(!answer)return;const hidden=answer.classList.toggle('hidden');e.target.textContent=hidden?'查看答案与解析':'收起答案与解析';});
