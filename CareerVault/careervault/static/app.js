...
function expCard(x){const details=x.details||{},type=x.type||'other';let title=x.title||'未命名记录',meta='',description='';
  if(type==='project'){meta=[experienceDateText(x),x.role].filter(Boolean).join(' · ');description=details.project_pitch||x.summary||x.facts||'';}
  ...
  return `<div class="card experience-card" data-id="${esc(x.id)}"><div class="row"><strong>${esc(title)}</strong><span class="status">${esc(TYPE_LABELS[type]||type||'其他')}</span></div>${meta?`<div class="muted">${esc(meta)}</div>`:''}${description?`<div class="experience-brief">${esc(compactText(description))}</div>`:''}${type==='project'?`${details.personal_contribution?`<div class="experience-brief"><strong>我做了什么：</strong>${esc(compactText(details.personal_contribution,100))}</div>`:''}${details.project_outcome?`<div class="muted">${esc(compactText(details.project_outcome,100))}</div>`:''}${!details.project_pitch&&!details.personal_contribution&&!details.project_outcome?'<div class="muted">旧版摘要 · 点击整理个人贡献与结果</div>':''}`:''}</div>`}
...
function experienceFields(x={}){const type=x.type||'project',details=x.details||{};
  if(type==='project')return `<label class="full">项目名称<input data-exp-field id="f-title" value="${esc(x.title||'')}" required></label>${dateRangeFields(x)}<label class="full">我的角色<input data-exp-field id="f-role" value="${esc(x.role||'')}" placeholder="例如：需求提出与试用迭代（AI 辅助开发）"></label>
  <div class="full muted">先让 HR 看懂你做了什么。以下三项用于项目卡片和简历，每项建议一句话；个人贡献最多两个重点。保留真实分工和完成范围。</div>
  <label class="full">做什么：一句话介绍<textarea data-exp-field data-brief id="f-project-pitch" rows="2" placeholder="为谁解决什么问题，做了什么工具或系统">${esc(details.project_pitch||'')}</textarea></label>
  <label class="full">我做了什么：核心贡献<textarea data-exp-field data-brief id="f-personal-contribution" rows="3" placeholder="我发现了什么、提出或完成了什么，不列全部功能">${esc(details.personal_contribution||'')}</textarea></label>
  <label class="full">做到什么程度：当前结果<textarea data-exp-field data-brief id="f-project-outcome" rows="2" placeholder="已交付、实际使用或验证到哪一步；没有数据就不写提升比例">${esc(details.project_outcome||'')}</textarea></label>
  <div class="full muted" id="briefHint" aria-live="polite"></div>
  <details class="full"><summary>HR 口述练习 · 约 30 秒</summary><label>口述稿<textarea data-exp-field id="f-hr-intro" placeholder="这是一个……。我主要做了……。比如……。目前……。">${esc(details.hr_intro||'')}</textarea></label><p class="muted">先讲用途、个人动作、一个例子和结果，然后停下来等追问。口述稿不写入简历。</p></details>
  <details class="full"><summary>详细事实与证据 · 追问时展开</summary><div class="muted">已有长文保留在这里。精简字段有内容后，简历以精简字段为准；请把必要的分工和验证边界也写进精简版。</div><label>项目背景与技术说明<textarea data-exp-field id="f-summary">${esc(x.summary||'')}</textarea></label><label>完整职责与过程<textarea data-exp-field id="f-facts">${esc(x.facts||details.project_role||'')}</textarea></label><label>成果与验证记录<textarea data-exp-field id="f-results">${esc(x.results||'')}</textarea></label><label>待核实与补充说明<textarea data-exp-field id="f-notes">${esc(x.notes||'')}</textarea></label></details>`;
...
async function openExperience(id, seed={}){
  ...
  $('#f-type').addEventListener('change',()=>{const type=$('#f-type').value;editingExperience=payload(editingExperience.type||x.type||'project');$('#experienceFields').innerHTML=experienceFields({...editingExperience,type});editingExperience.type=type;wireExperienceFields();scheduleAutosave()});
}
async function closeModal(){if(currentExperience&&saveTimer){clearTimeout(saveTimer);saveTimer=null;try{if(!await saveExperience(false))return}catch(error){return}}editorSession++;$('#modal').classList.add('hidden');currentExperience=null;editingExperience={};clearTimeout(saveTimer);saveTimer=null}
function fieldValue(id){return $('#'+id)?.value.trim()||''}
function updateBriefHint(){const el=$('#briefHint');if(!el)return;const n=['f-project-pitch','f-personal-contribution','f-project-outcome'].map(id=>fieldValue(id).length);el.textContent=`介绍 ${n[0]} 字 · 贡献 ${n[1]} 字 · 结果 ${n[2]} 字`+(n.some(v=>v>90)?' · 有一项较长，建议只保留一个核心动作和必要边界。':' · 建议每项 35–70 字，不必凑满。')}
function wireExperienceFields(){updateBriefHint();$$('[data-brief]').forEach(el=>el.addEventListener('input',updateBriefHint));const current=$('#f-current'),end=$('#f-end');if(current&&end){current.onchange=()=>{end.disabled=current.checked;if(current.checked)end.value='';scheduleAutosave()}};$$('[data-exp-field]').forEach(el=>el.addEventListener('input',scheduleAutosave))}
function endValue(){return $('#f-current')?.checked?'至今':fieldValue('f-end')}
function payload(typeOverride){const type=typeOverride||$('#f-type').value,base=editingExperience;const p={type,status:base.status||'active',title:base.title||'',organization:base.organization||'',role:base.role||'',start:base.start||'',end:base.end||'',domains:base.domains||[],skills:base.skills||[],related_experience_ids:base.related_experience_ids||[],details:{...(base.details||{})},resume_ready:Boolean(base.resume_ready),summary:base.summary||'',facts:base.facts||'',results:base.results||'',notes:base.notes||''};
  if(type==='project'){Object.assign(p,{title:fieldValue('f-title'),start:fieldValue('f-start'),end:endValue(),role:fieldValue('f-role'),summary:fieldValue('f-summary'),facts:fieldValue('f-facts'),results:fieldValue('f-results'),notes:fieldValue('f-notes')});Object.assign(p.details,{project_pitch:fieldValue('f-project-pitch'),personal_contribution:fieldValue('f-personal-contribution'),project_outcome:fieldValue('f-project-outcome'),hr_intro:fieldValue('f-hr-intro')})}
...
async function saveExperience(create=false){const p=payload();if(!p.title)return alert('请填写经历名称');clearTimeout(saveTimer);saveTimer=null;const session=editorSession;const id=currentExperience;state('保存中…');try{const x=await api(create?'/api/experiences':'/api/experiences/'+id,{method:create?'POST':'PATCH',body:JSON.stringify(p)});if(session!==editorSession)return x;currentExperience=x.id;editingExperience={...x,details:{...(x.details||{})}};experiences=experiences.map(item=>item.id===x.id?x:item);$$('.experience-card').filter(card=>card.dataset.id===x.id).forEach(card=>{card.outerHTML=expCard(x)});wireExperienceCards();state('✓ 已保存 '+new Date().toLocaleTimeString());if(create){await renderExperiences();await openExperience(x.id)}return x}catch(error){if(session===editorSession){state('保存失败');alert(`保存失败，编辑窗口未关闭：${error.message}`)}throw error}}
...
function renderIntegration(){$('#integration').innerHTML=`<div class="topbar"><div><h1>JobPilot 集成</h1><div class="muted">CareerVault = 事实库，JobPilot = JD 匹配 / 简历生成 / 网申填写。</div></div></div><div class="card"><h3>本地 API</h3><div class="code">GET  http://127.0.0.1:8776/api/jobpilot/profile\nGET  http://127.0.0.1:8776/api/jobpilot/experiences?resume_ready=true\nPOST http://127.0.0.1:8776/api/jobpilot/context</div><p>JobPilot 对每个 JD 调用 context 接口，即可拿到基础资料和排序后的 Resume Ready 经历。</p></div>`}
...
function experienceMatchesSearch(item, query){const d=item.details||{};return !query||[item.title,item.organization,item.role,d.project_pitch,d.personal_contribution,d.project_outcome,...(item.skills||[]),...(item.domains||[])].join(' ').toLowerCase().includes(query)}
